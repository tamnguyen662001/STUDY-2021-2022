...
--================================================================================
-- CÂU 1 
SELECT A.MACT, A.HOTEN,A.SO,A.VITRI, CONVERT(DATE,A.NGAYSINH,103) ,A.DIACHI
FROM CAUTHU A
-- CÂU 2
SELECT A.MACT, A.HOTEN,A.SO,A.VITRI, CONVERT(DATE,A.NGAYSINH,103) ,A.DIACHI
FROM CAUTHU A
WHERE A.SO = 7 AND A.VITRI = N'TIỀN VỆ'
-- CÂU 3
SELECT A.TENHLV, CONVERT(DATE,A.NGAYSINH,103), A.DIACHI,A.DIENTHOAI
FROM HUANLUYENVIEN A
--CÂU 4
SELECT A.MACT, A.HOTEN,A.SO,A.VITRI, CONVERT(DATE,A.NGAYSINH,103) ,A.DIACHI
FROM CAUTHU A JOIN CAULACBO B ON A.MACLB = B.MACLB JOIN QUOCGIA C ON A.MAQG = C.MAQG
WHERE C.TENQG = N'VIỆT NAM' AND B.TENCLB = N'BECAMEX BÌNH DƯƠNG'
--CÂU 5
SELECT A.MACT, A.HOTEN,A.SO,A.VITRI, CONVERT(DATE,A.NGAYSINH,103) ,A.DIACHI
FROM CAUTHU A JOIN CAULACBO B ON A.MACLB = B.MACLB JOIN QUOCGIA C ON A.MAQG = C.MAQG
WHERE C.TENQG = N'BRAZIL' AND B.TENCLB = N'SHB ĐÀ NẴNG'
--CÂU 6
SELECT A.MACT, A.HOTEN,A.SO,A.VITRI, CONVERT(DATE,A.NGAYSINH,103) AS NGÁYINH ,A.DIACHI
FROM CAUTHU A JOIN CAULACBO B ON A.MACLB = B.MACLB JOIN SANVD C ON B.MASAN = C.MASAN
WHERE C.TENSAN = N'LONG AN'
--CÂU 7
SELECT B.MATRAN,  CONVERT(DATE,B.NGAYTD,103), C.TENSAN, A1.TENCLB , B.KETQUA , A2.TENCLB
FROM CAULACBO A1 JOIN TRANDAU B ON A1.MACLB = B.MACLB1 JOIN CAULACBO A2 ON A2.MACLB = B.MACLB2
				JOIN SANVD C ON C.MASAN = B.MASAN
WHERE B.NAM = 2019 AND B.VONG = 2


SELECT A.MATRAN, CONVERT(DATE,A.NGAYTD,103), B.TENSAN, C1.TENCLB, C2.TENCLB , A.KETQUA
FROM dbo.TRANDAU A
INNER JOIN dbo.SANVD B ON B.MASAN = A.MASAN
INNER JOIN dbo.CAULACBO C1 ON C1.MACLB = A.MACLB1
INNER JOIn dbo.CAULACBO C2 ON C2.MACLB = A.MACLB2
WHERE A.VONG = 2 AND A.NAM = 2019

--CÂU 8
SELECT A.MAHLV, A.TENHLV, YEAR(A.NGAYSINH) + '-' + MONTH(A.NGAYSINH)+'-'  + DAY(A.NGAYSINH) AS NGAYSINH, A.DIACHI,B.VAITRO, C.TENCLB
FROM HUANLUYENVIEN A JOIN HLV_CLB B ON A.MAHLV = B.MAHLV 
JOIN CAULACBO C ON B.MACLB = C.MACLB JOIN QUOCGIA D ON A.MAQG = D.MAQG
WHERE D.TENQG = N'Việt Nam'


--CÂU 8
SELECT A.MAHLV, A.TENHLV, CONVERT(DATE,A.NGAYSINH,103) AS NGAYSINH, A.DIACHI,B.VAITRO, C.TENCLB
FROM HUANLUYENVIEN A JOIN HLV_CLB B ON A.MAHLV = B.MAHLV 
JOIN CAULACBO C ON B.MACLB = C.MACLB JOIN QUOCGIA D ON A.MAQG = D.MAQG
WHERE D.TENQG = N'Việt Nam'
...
-- CÂU 9 
--- CHỈ 1 VÒNG
SELECT TOP(3) A.MACLB,B.TENCLB, SUM(A.DIEM) AS N'Tổng' 
FROM dbo.BANGXH A JOIN dbo.CAULACBO B ON B.MACLB = A.MACLB
WHERE A.VONG = 3 AND A.NAM = 2019
GROUP BY A.MACLB, B.TENCLB
ORDER BY SUM(A.DIEM) DESC


--CẢ GIẢI
SELECT TOP(3) A.MACLB, SUM(A.DIEM) AS N'Tổng' 
FROM dbo.BANGXH A JOIN dbo.CAULACBO B ON B.MACLB = A.MACLB
GROUP BY A.MACLB
ORDER BY SUM(A.DIEM) DESC

-- TÍNH ĐIỂM CÁC CLB TROGNC Ả GIẢI
SELECT A.MACLB, SUM(A.DIEM)
FROM BANGXH A
--WHERE A.NAM = 2019
GROUP BY A.MACLB
ORDER BY SUM(A.DIEM) DESC
...
--9.lấy tên 3 câu lạc bộ có điểm cao nhất trong vòng 3 năm 2019
SELECT TOP 3 TENCLB
...
--CÂU 10
SELECT A.MAHLV, A.TENHLV, A.NGAYSINH, A.DIACHI,B.VAITRO, C.TENCLB
FROM HUANLUYENVIEN A JOIN HLV_CLB B ON A.MAHLV = B.MAHLV 
JOIN CAULACBO C ON B.MACLB = C.MACLB JOIN TINH D ON C.MATINH = D.MATINH
WHERE D.TENTINH = N'BÌNH DƯƠNG'

--CÂU 11 SỐ CẦU THỦ MỖI CLB
SELECT A.TENCLB , COUNT(A.MACLB) AS SL_CAUTHU
FROM CAULACBO A JOIN CAUTHU B ON A.MACLB = B.MACLB
GROUP BY A.TENCLB

-- CÂU 12 SỐ CẦU THỦ NƯỚC NGOÀI MỖI CLB 
SELECT A.TENCLB , COUNT(B.MACT) AS SL_CAUTHU
FROM  CAULACBO A JOIN CAUTHU B ON A.MACLB = B.MACLB
WHERE B.MAQG <> 'VN'
GROUP BY A.TENCLB
-- CÂU 13 
SELECT A.MACLB ,A.TENCLB, C.TENSAN, COUNT(A.MACLB) AS SL_CAUTHU
FROM  CAULACBO A JOIN CAUTHU B ON A.MACLB = B.MACLB 
JOIN SANVD C ON A.MASAN = C.MASAN JOIN QUOCGIA D ON D.MAQG = B.MAQG
WHERE D.TENQG <> N'VIỆT NAM'
GROUP BY A.MACLB ,A.TENCLB, C.TENSAN
HAVING COUNT(A.MACLB) > 2

-- CÂU 14
SELECT C.TENTINH , (COUNT(B.MACT) )AS SL_TIENDAO
FROM CAULACBO A JOIN CAUTHU B ON A.MACLB = B.MACLB JOIN TINH C ON A.MATINH = C.MATINH
WHERE B.VITRI = N'TIỀN ĐẠO'
GROUP BY C.TENTINH
...
SELECT * FROM CAUTHU
-- CÂU 15
SELECT TOP(1) A.MACLB,B.TENCLB,C.TENTINH, SUM(A.DIEM) AS N'Tổng' 
FROM dbo.BANGXH A JOIN dbo.CAULACBO B ON B.MACLB = A.MACLB JOIN TINH C ON B.MATINH = C.MATINH
WHERE A.VONG = 3 AND A.NAM = 2019
GROUP BY A.MACLB, B.TENCLB,C.TENTINH
ORDER BY SUM(A.DIEM) DESC
-- CÂU 16 ,16 TRỞ ĐI CHƯA LÀM 
SELECT TENHLV
FROM HUANLUYENVIEN
...
WHERE MAQG='VN' AND HLV.MAHLV NOT IN (
	SELECT HLV_CLB.MAHLV FROM HLV_CLB  )

--CÂU 18
SELECT A.MACT, A.HOTEN, A.NGAYSINH, A.VITRI, A.DIACHI, B.MACLB, b.TENCLB,C.HANG
FROM CAUTHU A
JOIN CAULACBO B ON B.MACLB = A.MACLB
JOIN BANGXH C ON C.MACLB = A.MACLB
WHERE C.VONG = 3 AND C.NAM = 2019 AND (C.HANG < 3 OR C.HANG > 6)
...
WHERE VONG = 3 AND NAM = 2019 AND LEFT(HIEUSO, PATINDEX('%-%', HIEUSO) - 1) 
>= ALL (SELECT LEFT(HIEUSO, PATINDEX('%-%', HIEUSO) - 1))
-- CÂU 26 GIỐNG CÂU 13
SELECT CLB.MACLB, CLB.TENCLB, SVD.TENSAN, SVD.DIACHI, COUNT(CT.MACT) AS 'CAUTHUNUOCNGOAI'
FROM CAULACBO CLB JOIN SANVD SVD ON CLB.MASAN = SVD.MASAN JOIN CAUTHU CT ON CLB.MACLB = CT.MACLB
WHERE CT.MACT NOT IN (
	SELECT CT1.MACT
	FROM CAUTHU CT1
	WHERE CT1.MAQG LIKE 'VN'
)
...
WHERE NAM = 2019 AND VONG >= ALL (SELECT VONG FROM BANGXH)
	AND CONVERT(INT, LEFT(HIEUSO, PATINDEX('%-%', HIEUSO) - 1))
...
-- CÂU 28
SELECT td.NGAYTD,sd.TENSAN,td.MACLB1,td.MACLB2,td.KETQUA
FROM TRANDAU td join SANVD sd on td.MASAN = sd.MASAN
				join CAULACBO clb on sd.MASAN = clb.MASAN
				join BANGXH bxh on clb.MACLB = bxh.MACLB
WHERE bxh.HANG>=all (
SELECT bxh1.HANG
FROM BANGXH bxh1
WHERE bxh1.VONG = 3 and NAM = 2019
)
...
SELECT MACLB, TENCLB
FROM CAULACBO CLB
WHERE (SELECT COUNT(MATRAN) 
	FROM TRANDAU 
	WHERE CLB.MACLB IN (TRANDAU.MACLB1, TRANDAU.MACLB2) AND NAM = 2019) = 
